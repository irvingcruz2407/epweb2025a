    ...
    React.useEffect(() => {
        if (user) {
            const fetchDashboardData = async () => {
                setIsLoading(true);
                try {
                    const transResponse = await fetch('http://localhost:3001/get-transactions-for-user', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(user),
                    });
                    const transactionsResult = await transResponse.json();
                    if (transactionsResult.success) {
                        setAllTransactions(transactionsResult.transactions || []);
                    }

                    if (user.role === 'ADMINISTRADOR' || user.role === 'SUPERVISOR') {
                        const vendorsResponse = await fetch('http://localhost:3001/vendors');
